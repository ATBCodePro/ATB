...
jumpSelect.addEventListener("change", (e) => {
  ...
  // 1. Force the sort to "evo" so the jump is chronological
  if (sortSelect.value !== "evo") {
    sortSelect.value = "evo";
    // Trigger the existing sort logic
    sortSelect.dispatchEvent(new Event('change'));
  }

  // 2. Find the index of the first card that exceeds the threshold
  const targetIndex = filteredCards.findIndex(card => (card.EVO || 0) > threshold);

  // 3. If a card is found, scroll to its DOM element
  if (targetIndex !== -1) {
    const cardElements = grid.querySelectorAll(".card");
    const targetElement = cardElements[targetIndex];

    if (targetElement) {
      targetElement.scrollIntoView({
        behavior: "smooth",
        block: "center" // Positions the card in the middle of the iPad screen
      });
    }
  }
  
  // Reset the dropdown so the user can click the same category again later
  jumpSelect.value = "";
});
